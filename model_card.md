...
Instead of relying solely on a static, built-in knowledge base, the system could be enhanced to make API calls to a reputable, live source of pet care information (like a veterinary health database or a major pet care organization). Users could still upload their own documents, but by combining user files with a constantly updated, trusted external source, the answers would become significantly more comprehensive and reliable. Also the current retrival method could be upgraded to a semantic search system. This will would allow for the answers generated by the current system to be more accurate.